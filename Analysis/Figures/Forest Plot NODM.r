# =============================================================================
# Figure 6: Forest Plot — Predictors of New-Onset DM
# =============================================================================
...
df <- df %>%
  ...
  mutate(
    ...
    preop_exo_insuff = as.integer(preop_exo_insuff == 1)
  )
...
rows_A <- bind_rows(
  ...
  get_row(res_A, "preop_exo_insuff",             "Pre-op exocrine insufficiency"),
...
rows_B <- bind_rows(
  ...
  get_row(res_B, "preop_exo_insuff",             "Pre-op exocrine insufficiency"),
  header_row("Peri-operative Factors"),
...
is_model_hdr <- grepl("^Model [AB]", fp_data$Variable)   # for bold post-process
...
fp <- forest(
  ...
  lower        = fp_data$lo,
  upper        = fp_data$hi,
  ci_column    = 5,            # " " is the 5th column (N removed)
  ref_line     = 1,
  x_trans      = "log",
  ...
  ## Increase the last value to widen the CI plot area
  ## Increase the first value if Variable labels get clipped
...
ci_col_idx  <- 5   # " " plot column index (updated — N removed)
pval_col_idx <- 4  # P-Value column index  (updated — N removed)
...
path_png <- file.path(fig_out, "Figure6_ForestPlot_DM_Predictors.png")
path_pdf <- file.path(fig_out, "Figure6_ForestPlot_DM_Predictors.pdf")

## ADJUST OVERALL IMAGE SIZE (inches): width controls lateral space,
## height controls vertical space — increase if rows get cramped
png(path_png, width = 6, height = 9, units = "in", res = 600, bg = "white")
print(fp)
dev.off()

pdf(path_pdf, width = 6, height = 9, bg = "white")
print(fp)
dev.off()

cat("\u2713 Figure 6 PNG:", path_png, "\n")
cat("\u2713 Figure 6 PDF:", path_pdf, "\n\n")
...
cat(paste0(
  "Figure 6. Forest plot of multivariable predictors of new-onset diabetes ",
  "mellitus following pancreatectomy. ",
