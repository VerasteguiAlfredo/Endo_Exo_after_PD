...
# AJCC/NCCN Tumor Stage
df$tumor_stage <- dplyr::case_when(
  # Stage 0
  df$pt_stage_1 == "Tis" & df$pn_stage_1 == "N0"                        ~ "Stage 0",
  # Stage IA
  df$pt_stage_1 == "T1"  & df$pn_stage_1 == "N0"                        ~ "Stage IA",
  # Stage IB
  df$pt_stage_1 == "T2"  & df$pn_stage_1 == "N0"                        ~ "Stage IB",
  # Stage IIA
  df$pt_stage_1 == "T3"  & df$pn_stage_1 == "N0"                        ~ "Stage IIA",
  # Stage IIB
  df$pt_stage_1 %in% c("T1","T2","T3") & df$pn_stage_1 == "N1"          ~ "Stage IIB",
  # Stage III — N2 (T1-T3)
  df$pt_stage_1 %in% c("T1","T2","T3") & df$pn_stage_1 == "N2"          ~ "Stage III",
  # Stage III — T4 any N
  df$pt_stage_1 == "T4"                                                  ~ "Stage III",
  TRUE ~ NA_character_
)
df$tumor_stage <- factor(df$tumor_stage,
                         levels = c("Stage 0", "Stage IA", "Stage IB",
                                    "Stage IIA", "Stage IIB",
                                    "Stage III", "Stage IV"))

# Quick check
cat("=== NCCN Tumor Stage ===\n")
print(table(df$tumor_stage, useNA = "ifany"))
...
add_cat_all("AJCC/NCCN tumor stage",             "tumor_stage")  # replaces T/N stage
...
build_html_table <- function(tbl, header_rows, table_number, title_text, caption_text) {
  ...
  paste0(
    ...
    "  <style>\n",
    "    body   { font-family: 'Times New Roman', sans-serif; margin: 60px 80px;",
...
# =============================================================================
# Table 3: Incidence and Timing of Endocrine and Exocrine Dysfunction
# =============================================================================
...
# -----------------------------------------------------------------------------
# 2. RESECTION TYPE GROUPING
# Collapse into clinically meaningful groups for stratification
# -----------------------------------------------------------------------------

df <- df %>%
  mutate(
    resection_group = case_when(
      resection_type == "PPPD"                         ~ "PPPD",
      resection_type == "sPD"                          ~ "Standard PD",
      resection_type %in% c("Distal")                 ~ "Distal",
      resection_type %in% c("PP Total",
                             "Standard Total")        ~ "Total",
      resection_type == "Central"                      ~ "Central",
      TRUE                                             ~ "Other"
    ),
    resection_group = factor(resection_group,
                             levels = c("PPPD", "Standard PD",
                                        "Distal", "Total",
                                        "Central", "Other"))
  )

cat("=== Resection groups ===\n")
print(table(df$resection_group, useNA = "ifany"))

# -----------------------------------------------------------------------------
# 3. HELPER FUNCTIONS
# -----------------------------------------------------------------------------

med_iqr <- function(x, positive_only = FALSE) {
  if (positive_only) x <- x[!is.na(x) & x > 0]
  else               x <- x[!is.na(x)]
  if (length(x) == 0) return("\u2014")
  q <- quantile(x, probs = c(0.25, 0.5, 0.75))
  sprintf("%.1f (%.1f\u2013%.1f)", q[2], q[1], q[3])
...
med_iqr_months <- function(days_var, positive_only = TRUE) {
  x <- df[[days_var]]
  if (positive_only) x <- x[!is.na(x) & x > 0]
  else               x <- x[!is.na(x)]
  if (length(x) == 0) return("\u2014")
  x_mo <- x / 30.44
  q    <- quantile(x_mo, probs = c(0.25, 0.5, 0.75))
  sprintf("%.1f (%.1f\u2013%.1f)", q[2], q[1], q[3])
}

# -----------------------------------------------------------------------------
# 4. OUTCOME SUMMARY FUNCTION
# Returns a one-row summary for a given subset of df
# -----------------------------------------------------------------------------

outcome_row <- function(data, label) {
  n <- nrow(data)

  # Endocrine
  dm_n    <- sum(data$new_onset_dm == 1, na.rm = TRUE)
  dm_pct  <- ifelse(n > 0, sprintf("%d (%.1f%%)", dm_n, dm_n / n * 100), "\u2014")

  ins_n   <- sum(data$insulin == 1, na.rm = TRUE)
  ins_pct <- ifelse(n > 0, sprintf("%d (%.1f%%)", ins_n, ins_n / n * 100), "\u2014")

  dm_time <- {
    x <- data$days_to_dm[!is.na(data$days_to_dm) & data$days_to_dm > 0] / 30.44
    if (length(x) == 0) "\u2014"
    else {
      q <- quantile(x, probs = c(0.25, 0.5, 0.75))
      sprintf("%.1f (%.1f\u2013%.1f)", q[2], q[1], q[3])
  ...
  # Exocrine
  exo_n   <- sum(data$exo_insufficiency == 1, na.rm = TRUE)
  exo_pct <- ifelse(n > 0, sprintf("%d (%.1f%%)", exo_n, exo_n / n * 100), "\u2014")

  pert_n   <- sum(data$pert == 1, na.rm = TRUE)
  pert_pct <- ifelse(n > 0, sprintf("%d (%.1f%%)", pert_n, pert_n / n * 100), "\u2014")

  exo_time <- {
    x <- data$days_to_exo_insuff[!is.na(data$days_to_exo_insuff) &
                                    data$days_to_exo_insuff > 0] / 30.44
    if (length(x) == 0) "\u2014"
    else {
      q <- quantile(x, probs = c(0.25, 0.5, 0.75))
      sprintf("%.1f (%.1f\u2013%.1f)", q[2], q[1], q[3])
...
# -----------------------------------------------------------------------------
# 5. BUILD TABLE ROWS
# Overall + stratified by resection group
# -----------------------------------------------------------------------------

rows <- list()

# Overall
rows[[1]] <- outcome_row(df, "Overall")

# Separator
rows[[2]] <- data.frame(
  Group    = "Stratified by Resection Type",
  N = "", DM = "", DM_Time = "", Insulin = "",
  Exo = "", Exo_Time = "", PERT = "",
  stringsAsFactors = FALSE
)

# By resection group — only show groups with n >= 10
for (grp in levels(df$resection_group)) {
  sub <- df %>% filter(resection_group == grp)
  if (nrow(sub) >= 10) {
    rows[[length(rows) + 1]] <- outcome_row(sub, paste0("    ", grp))
  }
}
...
cat("\n", strrep("=", 78), "\n", sep = "")
cat("Table 3. Incidence and Timing of Endocrine and Exocrine Dysfunction\n")
cat(strrep("=", 78), "\n")
print(kable(tbl3, format = "markdown", align = c("l","r","c","c","c","c","c","c")))
cat("\nContinuous variables: Median (Q1-Q3) in months. Categorical: No. (%).\n\n")
...
ht3 <- kable(tbl3,
             format  = "html",
             align   = c("l","r","c","c","c","c","c","c"),
             escape  = FALSE,
             caption = paste0(
               "<b>Table 3.</b> Incidence and Timing of Endocrine and Exocrine ",
               "Dysfunction Following Pancreatectomy (N = ", total_n, ")"
             )) %>%
  kable_styling(
    bootstrap_options = c("condensed"),
    full_width        = TRUE,
  ...
  column_spec(3:8, width = "12%",
              extra_css = "text-align: center !important;
                           border-left: none !important; border-right: none !important;") %>%
  add_header_above(c(
    " "        = 2,
    "Endocrine Outcomes" = 3,
    "Exocrine Outcomes"  = 3
  ),
  bold      = TRUE,
  extra_css = paste0(
  ...
  footnote(
    general = paste0(
      "Data are presented as number (percentage) for incidence outcomes and ",
      "median (interquartile range) in months for time-to-event outcomes. ",
      "Time-to-event calculated from date of surgery to first documented diagnosis. ",
      "Patients with pre-operative exocrine insufficiency were retained and flagged ",
      "as a covariate (n = ", sum(df$preop_exo_insuff == 1, na.rm = TRUE), "). ",
      "Resection groups with fewer than 10 patients are not shown separately. ",
      "DM = diabetes mellitus; PERT = pancreatic enzyme replacement therapy; ",
      "IQR = interquartile range; PPPD = pylorus-preserving pancreaticoduodenectomy."
    ),
...
html3 <- paste0(
  "<!DOCTYPE html>\n<html>\n<head>\n",
  "  <meta charset='UTF-8'>\n",
  "  <title>Table 3 \u2013 Endocrine and Exocrine Dysfunction</title>\n",
  "  <style>\n",
  "    body   { font-family: 'Times New Roman', sans-serif; margin: 60px 80px;",
  "             max-width: 960px; color: #111; background: #fff; }\n",
  "    caption { font-size: 13px; margin-bottom: 6px;",
  "              text-align: left; color: #111; caption-side: top; }\n",
  "    table  { border-collapse: collapse; width: 100%; }\n",
  "    td, th { padding: 4px 8px;",
...
path3 <- file.path(out_dir, "Table3_Endocrine_Exocrine_Dysfunction.html")
writeLines(html3, path3)

cat("\u2713 Table 3 exported to:\n  ", path3, "\n\n")